<template>
  <app-container>
    ...
    <search-area size="small" @search="search">
      ...
      <template slot="btns">
        <el-button v-if="hasPerm('/sys/loginLog/delLoginLog')" class="filter-item" size="small" type="info" icon="el-icon-delete" @click="cleanAll">
          清空日志
        </el-button>
      </template>
    </search-area>
...
<script>
import { getLoginLogList, delLoginLog } from '@/api/system/log'
export default {
  name: 'LoginLog',
  data() {
    return {
  ...
  methods: {
    ...
    cleanAll() {
      this.$confirm(
        ...
        delLoginLog(this.query).then(response => {
